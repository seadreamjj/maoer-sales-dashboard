...
def main():
    ...
    cards=''.join(f'<a class="board-card" href="{k}.html"><div class="eyebrow">{k.upper()}</div><h2>{c["label"]}</h2><p>{df.日期.min()} → {df.日期.max()} · {len(df):,} 条记录</p><span>进入分析 →</span></a>' for k,c in BOARDS.items() if (df:=boards.get(k)) is not None)
    (OUTPUT_DIR/"index.html").write_text(f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>猫耳数据中心</title>{CSS}</head><body>{nav('home')}<div class="container"><section class="hero"><div class="eyebrow">MAOER · DATA CENTER</div><h1>猫耳数据中心</h1><p>销量、热度与新品历史榜单 · 以剧集 ID 为核心识别</p></section><div class="board-grid">{cards}</div><div class="footer">数据由 GitHub Actions 自动更新</div></div></body></html>''',encoding='utf-8')
