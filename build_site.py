...
def generate_index_page(full_df, default_start, default_end):
...
def run_analysis():
    ...
    earliest_date = pd.to_datetime(all_dates[0])

    default_start_dt = latest_date - pd.Timedelta(days=DEFAULT_DAYS - 1)
    if default_start_dt < earliest_date:
        default_start_dt = earliest_date
    default_start = default_start_dt.strftime("%Y-%m-%d")
    default_end = latest_date.strftime("%Y-%m-%d")

    start_date = os.environ.get("ANALYSIS_START", default_start)
    end_date = os.environ.get("ANALYSIS_END", default_end)
    ...
    selected_mask = (full_df["日期"] >= start_date) & (full_df["日期"] <= end_date)
    if not selected_mask.any():
        raise ValueError(f"{start_date} → {end_date} 没有数据")

    filtered_df = full_df[selected_mask].copy()
    actual_first_date = filtered_df["日期"].min()
    actual_last_date = filtered_df["日期"].max()
    summary = build_summary(filtered_df, actual_first_date, actual_last_date)

    clean_output_directory()
    index_path = generate_index_page(full_df, default_start, default_end)

    excel_path = generate_excel(filtered_df, summary, start_date, end_date)
    ...
    metadata = {
        ...
        "drama_count": int(full_df["剧名"].nunique()),
        "record_count": int(len(full_df)),
        "architecture": "single-page-dashboard-json",
        "delta_modes": ["7日增量", "每日增量"],
    }
    (OUTPUT_DIR / "build-info.json").write_text(
        json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    print("🎉 GitHub Pages 网站构建完成")
    print(f"🌐 网站目录：{OUTPUT_DIR}")
    print(f"📊 Excel：{excel_path}")
    return {"full_df": full_df, "filtered_df": filtered_df, "summary": summary, "index_path": index_path, "excel_path": excel_path}
